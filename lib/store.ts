...
export const normalizePersistedState = (
  input: Partial<PersistedState> | null | undefined,
  fallback: PersistedState = fallbackState()
): PersistedState => {
  const parsed = input ?? {};
  return {
    entries: Array.isArray(parsed.entries) && parsed.entries.length ? parsed.entries : fallback.entries,
    notifications:
      Array.isArray(parsed.notifications) && parsed.notifications.length ? parsed.notifications : fallback.notifications,
    loginLogs:
...
export const getPersistedState = (): PersistedState => {
  ...
  try {
    ...
    return normalizePersistedState(parsed, fallback);
  } catch {
    return fallback;
  }
};
